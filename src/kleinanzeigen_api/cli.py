...
def _cmd_chats(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api chats", description="List your chat threads."
    )
...
def _cmd_reply(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api reply", description="Reply in a chat thread."
    )
...
def _cmd_messages(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api messages", description="Show messages in a chat thread."
    )
    ap.add_argument("conversation_id")
    ap.add_argument("--json", action="store_true")
    a = ap.parse_args(argv)
...
def _cmd_my_ads(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api my-ads", description="List your own ads."
    )
    ap.add_argument("--json", action="store_true")
    a = ap.parse_args(argv)
    api = _authed_client()
    ads = api.my_ads()
    if a.json:
        print(json.dumps([ad.to_dict() for ad in ads], ensure_ascii=False, indent=2))
    else:
        _print_table(ads)
...
def _cmd_watchlist(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api watchlist", description="List ads on your watchlist."
    )
    ap.add_argument("--json", action="store_true")
    a = ap.parse_args(argv)
    api = _authed_client()
...
def _cmd_post(argv) -> int:
    ap = argparse.ArgumentParser(
        prog="kleinanzeigen-api post", description="Post a new ad."
    )
    ...
    ap.add_argument(
        "--price-type", default="FIXED", choices=["FIXED", "NEGOTIABLE", "FREE"]
    )
...
def _cmd_watch(argv) -> int:
    ...
    ap.add_argument(
        "--category", metavar="NAME_OR_ID", help="restrict to a category by name or id"
    )
    ...
    ap.add_argument(
        "--distance", type=float, metavar="KM", help="radius in km around --location"
    )
    ap.add_argument(
        "--near",
        metavar="LAT,LON",
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="min seconds between requests (default: 1.5, or 0.05 " "with --fast)",
    )
    ...
    if a.fast and a.location:
        print(
            "--location only works in the default mode; with --fast use "
            "--near LAT,LON --radius KM instead",
            file=sys.stderr,
        )
        return 2
    ...
    try:
        for ad in api.iter_new_ads(
            ...
            if a.json:
                print(json.dumps(ad.to_dict(), ensure_ascii=False), flush=True)
            else:
                price = f"{int(ad.price)} €" if ad.price else (ad.price_type or "—")
                loc = f"{ad.zip_code} {ad.city}".strip() or "—"
                print(f"[{ad.id}] {price:>9} | {loc}  {ad.title[:70]}", flush=True)
                print(f"    {ad.url}", flush=True)
...
def _print_table(items: list) -> None:
    ...
    for item in items:
        price = f"{int(item.price)} €" if item.price else (item.price_type or "—")
        loc = f"{item.zip_code} {item.city}".strip() or "—"
        extra = []
        if item.rooms:
            extra.append(f"{item.rooms:g} Zi")
        if item.size_m2:
            extra.append(f"{item.size_m2:g} m²")
        nk = item.attributes.get("Warmmiete")
        if nk:
            extra.append(f"warm {nk}€")
        tail = (" | " + " · ".join(extra)) if extra else ""
        print(f"[{item.id}] {price:>9} | {loc}{tail}")
...
def main(argv=None) -> int:
    ...
    p.add_argument(
        "location", nargs="?", help="city/region name or numeric location id"
    )
    p.add_argument(
        "--category",
        default=None,
    ...
    p.add_argument(
        "--sort",
        choices=["new", "cheap", "expensive", "near"],
    ...
    p.add_argument(
        "--rate", type=float, default=1.5, help="min seconds between requests"
    )
    ...
    sort_type = sort_map.get(args.sort) or (
        "PRICE_ASCENDING" if args.sort_price else None
    )

...
def test_resolve_location_api_flattens_tree(monkeypatch):
    ...
    monkeypatch.setattr(
        api, "_get", lambda url, params=None: _Resp(_locations_payload())
    )
...
def test_resolve_location_falls_back_to_website(monkeypatch):
    ...
    monkeypatch.setattr(
        api, "_resolve_location_web", lambda q: [("999", "Fallbackcity - State")]
    )
    ...
    return {
        SEARCH_META_NS: {
            "value": {
                "q": {
                    "type": "STRING",
                    "localized-label": "Titel",
                    "search-param": "optional",
                },
                "minPrice": {
                    "type": "DECIMAL",
                    "localized-label": "Preis von",
                    "search-param": "optional",
                },
                "priceType": {
                    "type": "ENUM",
                    "localized-label": "Preis",
...
def test_search_metadata_parses_params_and_enums(monkeypatch):
    api = KleinanzeigenAPI()
    monkeypatch.setattr(
        api, "_get", lambda url, params=None: _Resp(_metadata_payload())
    )
    meta = api.search_metadata(category_id=203)
    assert meta["minPrice"] == {
        "label": "Preis von",
        "type": "DECIMAL",
        "search_param": "optional",
    }
    assert meta["priceType"]["values"] == [
        ("FREE", "Zu verschenken"),

...
def test_excluded_matches_title_or_description():
    listing = mk("1", "iPhone 13", "voll funktionsfähig")
    assert _excluded(listing, ["iphone"]) is True
    assert _excluded(listing, ["defekt"]) is False
    assert _excluded(mk("2", "Pixel 7", "leider defekt"), ["defekt"]) is True
    assert _excluded(listing, []) is False
...
def test_search_applies_exclude(monkeypatch):
    ...
    assert [listing.id for listing in out] == ["3"]
...
def test_search_none_location_is_allowed(monkeypatch):
    ...
    captured = {}
    api = KleinanzeigenAPI()
    monkeypatch.setattr(api, "search_page", lambda **kw: captured.update(kw) or (0, []))
...
def test_search_dedupes_across_pages(monkeypatch):
    ...
    assert [listing.id for listing in out] == ["9"]  # seen-set prevents duplicates
